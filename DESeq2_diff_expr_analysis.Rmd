...
library(org.Rn.eg.db) # Rat's Annotationdbi --> must be installed from Bioconductor
register(MulticoreParam(4)) # Change this based on your computer core count
```

Set colors from colorbrewer
```{r}
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
mycols = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
```

Prepare columns name, and dataframe for analysis
```{r}
sample_1 <- "Neuropil"
sample_2 <- "Somata"
n_rep <- 3

sample_column <- c(rep(sample_1, n_rep), rep(sample_2, n_rep))

run_column <- c(paste(sample_1, "1", sep = "_"), paste(sample_1, "2", sep = "_"), paste(sample_1, "3", sep = "_"),
                paste(sample_2, "1", sep = "_"), paste(sample_2, "2", sep = "_"), paste(sample_2, "3", sep = "_"))

rep_column <- c(rep(c("A", "B", "C"), 2))
...
samples_df$condition <- factor(rep(c(sample_1, sample_2), each = n_rep))
...
```{r}
dds$condition <- factor(c(rep(sample_1, n_rep),
                          rep(sample_2, n_rep)),
                        levels = c(sample_1,
                                   sample_2))
```
...
We need to perform a log transform for data quality assement 
```{r}
...
Create sample distance matrix image as pdf 
```{r}
...
Create count matrix heat map as pdf
```{r}
...
Using contrast to we can scale up the number of samples
```{r}
